...
class Cell {
	 ...
	 * up, it is necessary to erase the (single) line afterward so that
	 * subsequent searches for isolines will not loop indefinitely.
	 */
	void clear() {
		switch (cellNdx) {
		case 0:
	...
	public String toString() {
		return new StringBuilder("Cell{index=").append(cellNdx).append(", flipped? ").append(flipped).append(", left=")
				.append(left).append(", top=").append(top).append(", right=").append(right).append(", bottom=")
				.append(bottom).append('}').toString();
	}
