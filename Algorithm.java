...
//				ndx |= (bl > isovalue ? 0 : 1);
	...
	ArrayList<ArrayList<Double>> buildContours2(final double[][] data, final double[] levels)
			throws InterruptedException, ExecutionException {
		this.isovalues = levels;
		long t = -System.currentTimeMillis();
		// find min, max, and guard
	...
	private ArrayList<ArrayList<Double>> doConcurrent2(final double[][] data) throws InterruptedException, ExecutionException {
		final Collection<Callable<Result>> workers = new ArrayList<>();
		for (int i = 0; i < isovalues.length; i++) {
			workers.add(new Task(i, data, isovalues[i]));
		}
	...
	private static final class Result {
		...
		Result(final int ndx, final GeneralPath path,ArrayList<ArrayList<Double>> contours) {
			super();
			this.ndx = ndx;
			this.path = path;
		...
		public String toString() {
			if (str == null) {
				str = new StringBuilder("Result{ndx=").append(ndx).append(", bbox=").append(path.getBounds())
						.append('}').toString();
			}
			return str;
		}
	...
	private final class Task implements Callable<Result> {
		...
		public Result call() throws Exception {
			...
			try {
				...
				path = pG.generate(contour(data, level));
			} catch (Exception x) {
			...
			return new Result(ndx, path,pG.getContours());
		}
	}
}
